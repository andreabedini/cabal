...
instance (Typeable key, Structured ipkg, Structured srcpkg) => Structured (GenericInstallPlan' key ipkg srcpkg) where
  structure p =
    Nominal
      (typeRep p)
      0
...
showInstallPlan
  :: forall ipkg srcpkg key
   . ( Package ipkg, IsNode ipkg, Key srcpkg ~ key
     , Package srcpkg, IsNode srcpkg, Key ipkg ~ key
     , Pretty key
     )
  => GenericInstallPlan ipkg srcpkg
  -> String
...
-- already installed too (that is 'PreExisting' or 'Installed').
installedM
  :: (IsNode ipkg, IsNode srcpkg, Key ipkg ~ Key srcpkg, Monad m)
  => (srcpkg -> m Bool)
  -> GenericInstallPlan ipkg srcpkg
...
lookup
  :: (IsNode ipkg, IsNode srcpkg, Key ipkg ~ k, Key srcpkg ~ k)
  => GenericInstallPlan ipkg srcpkg
  -> k
...
project
  ...
  => ( IsNode ipkg1, Key ipkg1 ~ keyA
     , IsNode srcpkg1, Key srcpkg1 ~ keyA
     , IsNode ipkg2, Key ipkg2 ~ keyB
     , IsNode srcpkg2, Key srcpkg2 ~ keyB
     , Show keyB, Pretty keyB
    )
  => (ipkg1 -> Maybe ipkg2)
  -> (srcpkg1 -> Maybe srcpkg2)
  -> GenericInstallPlan ipkg1 srcpkg1
  -> GenericInstallPlan ipkg2 srcpkg2
project f g =
   mkInstallPlan "project" . 
   Graph.fromDistinctList .
   mapMaybe (bitraverse f g)
   . toList
