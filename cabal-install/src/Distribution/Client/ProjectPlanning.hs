      ...
      $ do
          ...
          (compiler, Platform arch os, _) <-
            phaseConfigureCompiler verbosity
                                   distDirLayout
                                   ((fst $ PD.ignoreConditions projectConfigSkeleton) <> cliConfig)
...
rebuildInstallPlan verbosity
    ...
    runRebuild distProjectRootDirectory $ do
      ...
      let fileMonitorImprovedPlan = newFileMonitor (distProjectCacheFile "improved-plan")
      rerunIfChanged verbosity
        ...
        (elaboratedPlan, elaboratedShared, totalIndexState, activeRepos) <- do
          let fileMonitorElaboratedPlan = newFileMonitor (distProjectCacheFile "elaborated-plan")
          rerunIfChanged verbosity
            ...
            -- it independently.
            compilerEtc   <- phaseConfigureCompiler verbosity distDirLayout projectConfig

            _             <- phaseConfigurePrograms projectConfig compilerEtc

            (solverPlan, pkgConfigDB, totalIndexState, activeRepos)
                          <- phaseRunSolver verbosity
                                            distDirLayout
                                            projectConfig
                                            compilerEtc
                                            localPackages
...
phaseRunSolver verbosity
    ...
    rerunIfChanged verbosity fileMonitorSolverPlan
      ...
      liftIO $ do
        ...
        planOrError <- foldProgress logMsg (pure . Left) (pure . Right) $
          planPackages verbosity compiler platform solverSettings
  ...
  where
    fileMonitorSolverPlan = newFileMonitor (distProjectCacheFile distDirLayout "solver-plan")
...
phaseElaboratePlan :: Verbosity
                   ...
                   -> Rebuild ( ElaboratedInstallPlan
                              , ElaboratedSharedConfig )
phaseElaboratePlan verbosity distDirLayout storeDirLayout ProjectConfig {
                     projectConfigShared,
  ...
  where
    fileMonitorSourceHashes = newFileMonitor (distProjectCacheFile distDirLayout "source-hashes")
    withRepoCtx = projectConfigWithSolverRepoContext verbosity
                    projectConfigShared
                    projectConfigBuildOnly
...
--
phaseMaintainPlanOutputs :: Verbosity
                         -> DistDirLayout
                         -> ElaboratedInstallPlan
                         -> ElaboratedSharedConfig
                         -> Rebuild ()
...
--
phaseImprovePlan :: Verbosity
                 -> StoreDirLayout
                 -> ElaboratedInstallPlan
                 -> ElaboratedSharedConfig
                 -> Rebuild ElaboratedInstallPlan
phaseImprovePlan verbosity storeDirLayout elaboratedPlan elaboratedShared = do

    liftIO $ debug verbosity "Improving the install plan..."
    storePkgIdSet <- getStoreEntries storeDirLayout compid
...
planPackages verbosity comp platform SolverSettings{..}
             installedPkgIndex sourcePkgDb pkgConfigDB
