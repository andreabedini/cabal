...
configureToolchains
  ...
  ProjectConfig
    ...
    let fileMonitorCompiler = newFileMonitor $ distProjectCacheFile "compiler"
        fileMonitorDefaultProgramDb = newFileMonitor $ distProjectCacheFile "programdb"
    ...
    progdb <-
      rerunIfChanged
        verbosity
        fileMonitorDefaultProgramDb
        ( progsearchpath
        , packageConfigProgramPaths
        , packageConfigProgramPathExtra
        )
        $ liftIO
        $ resolveProgramDb verbosity projectConfigLocalPackages
      ...
      rerunIfChanged
        ...
        $ do
          ...
          (compiler, platform, programDb) <-
            liftIO $ Cabal.configCompiler buildHcFlavor buildHcPath progdb verbosity
          ...
          monitorFiles (programsMonitorFiles programDb)
    ...
    (hostHc, hostPlat, hostHcProgDb) <-
      rerunIfChanged
        ...
        $ do
          ...
          liftIO $ info verbosity "Compiler settings changed, reconfiguring..."
          (compiler, platform, programDb) <-
            liftIO $ Cabal.configCompiler hostHcFlavor hostHcPath progdb verbosity
            -- Note that we added the user-supplied program locations and args
            -- for /all/ programs, not just those for the compiler prog and
            -- compiler-related utils. In principle we don't know which programs
            -- the compiler will configure (and it does vary between compilers).
          ...
          buildComponent (cc_map, lc_map, exe_map) comp =
            addProgressCtx
              ...
              $ do
                ...
                infoProgress $ hang (text "configuring component" <+> pretty cname) 4 $ vcat
...
instantiateInstallPlan storeDirLayout defaultInstallDirs elaboratedShared plan = do
  ready_map <- flip execStateT Map.empty $ do
    for_ pkgs $ \case
  ...
  let g = Graph.fromDistinctList (Map.elems ready_map)
  
  for_ (Graph.broken g) $ \(pkg, deps) ->
    infoProgress $ hang (pretty (Graph.nodeKey pkg)) 4 $ vcat (map pretty deps)
  
  return $! InstallPlan.new g
  where
