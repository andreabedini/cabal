...
{- Note [No non-reinstallable constraints]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Upstream cabal-install marks a set of "non-reinstallable" / wired-in packages
(base, ghc, ghc-internal, ghc-prim, template-haskell, ...) and adds a HARD
solver constraint forcing them to resolve to the compiler's *installed*
instance (PackagePropertyInstalled, or, for wired-in units,
PackagePropertyInstalledSpecificUnitId tied to the compiler's reported unit
id). The rationale upstream is that you cannot rebuild e.g. base for a fixed
GHC from a Hackage sdist.

We deliberately DO NOT do this. The constraint is wrong for a staged GHC build
and we rely on per-stage indices + preferences instead. The intended design:

  * Build stage (the boot/bootstrap compiler): used to compile Setup.hs and
    build-tool executables that must run *now*. Its core libraries are fixed and
    cannot be rebuilt, so we want to reuse the boot compiler's *installed* base,
    ghc, ghc-internal, ... — but as a PREFERENCE, not a hard constraint. The
    default solver preference 'PreferLatestForSelected' already prefers installed
    instances for non-target packages, so build-stage dependencies pick up the
    boot compiler's libraries. A user who genuinely wants to build a core package
    from source in the build stage can opt in with an explicit 'source'
    constraint.

  * Host stage (the stage we are producing): here the core libraries
    (base, ghc, ghc-internal, rts, ...) ARE the deliverables and are built FROM
    SOURCE by the host compiler. The host stage's installed index has the
    source-package counterparts removed (see 'installedPkgIndex'' and
    'hideInstalledPackagesSpecificBySourcePackageId') and, in a from-scratch GHC
    build, is typically empty of these packages anyway — so the solver builds
    them from source.

The unifying principle: a package that appears as a local *source* package in
the project is meant to be built; we must not force it to its installed version.
A hard non-reinstallable/wired-in constraint contradicts this directly and, for
an in-tree host compiler (which can also report a bogus
'ghc-internal Unit Id: <unavailable>' and has an empty installed index), is
simply unsatisfiable — which is exactly the staged-build solver failure this
note exists to prevent.

DO NOT reintroduce dependOnWiredIns / dontInstallNonReinstallablePackages when
rebasing onto upstream. (Cabal still populates Compiler.compilerWiredInUnitIds;
it is just no longer turned into a solver constraint here.)
-}
...
resolveDependencies toolchains pkgConfigDB installedPkgIndex params =
  ...
  where
    finalparams@( DepResolverParams
        ...
        -- See Note [No non-reinstallable constraints]
        params
