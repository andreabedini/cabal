...
tests =
  ...
  , testGroup
      ...
      , let checkFullLog =
         ...
         in runTest $
              setVerbose $
                constraints [ExVersionConstraint (ConstraintScope (ScopeAnyQualifier "true-dep")) V.noVersion] $
      ...
      , let cs = [ExFlagConstraint (ConstraintScope (ScopeAnyQualifier "pkg")) "flag" False]
         in runTest $
              constraints cs $
  ...
  , testGroup
      ...
      [ let name = "Top-level flag constraint does not constrain setup dep's flag"
            cs = [ExFlagConstraint (ConstraintScope (ScopeQualified P.QualToplevel "B")) "flag" False]
         in runTest $
              constraints cs $
      ...
      , let name = "Solver can toggle setup dep's flag to match top-level constraint"
            cs =
              [ ExFlagConstraint (ConstraintScope (ScopeQualified P.QualToplevel "B")) "flag" False
              , ExVersionConstraint (ConstraintScope (ScopeAnyQualifier "b-2-true-dep")) V.noVersion
              ]
         in runTest $
              constraints cs $
                mkTest dbSetupDepWithManualFlag name ["A"] $
      ...
      , let name = "User can constrain flags separately with qualified constraints"
            cs =
              [ ExFlagConstraint (ConstraintScope (ScopeQualified P.QualToplevel "B")) "flag" True
              , ExFlagConstraint (ConstraintScope (ScopeQualified (P.QualSetup "A") "B")) "flag" False
              ]
         in runTest $
              constraints cs $
                mkTest dbSetupDepWithManualFlag name ["A"] $
  ...
  , testGroup
      ...
      , let cs = [ExVersionConstraint (ConstraintScope (ScopeAnyQualifier "D")) $ mkVersionRange 1 4]
         in runTest $
              constraints cs $
                mkTest dbConstraints "force older versions with unqualified constraint" ["A", "B", "C"] $
                  solverSuccess [("A", 1), ("B", 2), ("C", 3), ("D", 1), ("D", 2), ("D", 3)]
      , let cs =
              [ ExVersionConstraint (ConstraintScope (ScopeQualified P.QualToplevel "D")) $ mkVersionRange 1 4
              , ExVersionConstraint (ConstraintScope (ScopeQualified (P.QualSetup "B") "D")) $ mkVersionRange 4 7
              ]
         in runTest $
              constraints cs $
                mkTest dbConstraints "force multiple versions with qualified constraints" ["A", "B", "C"] $
                  solverSuccess [("A", 1), ("B", 5), ("C", 9), ("D", 1), ("D", 5), ("D", 9)]
      , let cs = [ExVersionConstraint (ConstraintScope (ScopeAnySetupQualifier "D")) $ mkVersionRange 1 4]
         in runTest $
              constraints cs $
  ...
  , testGroup
      ...
      , let db =
         ...
         in runTest $
              constraints [ExFlagConstraint (ConstraintScope (ScopeAnyQualifier "B")) "make-lib-private" True] $
  ...
  , -- tests for partial fix for issue #5325
    testGroup "Components that are unbuildable in the current environment" $
      let flagConstraint = ExFlagConstraint . ConstraintScope . ScopeAnyQualifier
...
chooseUnbuildableExeAfterBuildToolsPackage name =
  constraints [ExFlagConstraint (ConstraintScope (ScopeAnyQualifier "B")) "build-bt2" False] $
    goalOrder goals $
      mkTest db name ["A"] $
        solverFailure $
          isInfixOf $
...
setupStanzaTest2 =
  constraints [ExStanzaConstraint (ConstraintScope (ScopeAnyQualifier "B")) [TestStanzas]] $
    mkTest
      dbSetupStanza
      "setupStanzaTest2"
      ["A"]
