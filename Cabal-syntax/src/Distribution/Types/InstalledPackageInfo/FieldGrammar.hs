...
setMaybePackageName (Just pn) ipi =
  ipi
...
setMungedPackageName (MungedPackageName pn ln) ipi =
  ipi
    { sourcePackageId = (sourcePackageId ipi){pkgName = pn}
    , sourceLibName = ln
    }
...
basicLibName f b =
  (\x -> b{_basicLibName = maybeToLibraryName x})
    <$> f (libraryNameString (_basicLibName b))
{-# INLINE basicLibName #-}

basicLibVisibility :: Lens' Basic LibraryVisibility
basicLibVisibility f b =
  (\x -> b{_basicLibVisibility = x})
