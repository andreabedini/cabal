...
addExtraCSources bi extras = bi{cSources = new}
  where
    new = ordNub (map (`ExtraSourcePkg` []) extras ++ cSources bi)
...
addExtraCxxSources bi extras = bi{cxxSources = new}
  where
    new = ordNub (map (`ExtraSourcePkg` []) extras ++ cxxSources bi)
...
addExtraCmmSources bi extras = bi{cmmSources = new}
  where
    new = ordNub (map (`ExtraSourcePkg` []) extras ++ cmmSources bi)
...
addExtraAsmSources bi extras = bi{asmSources = new}
  where
    new = ordNub (map (`ExtraSourcePkg` []) extras ++ asmSources bi)
...
addExtraJsSources bi extras = bi{jsSources = new}
  where
    new = ordNub (map (`ExtraSourcePkg` []) extras ++ jsSources bi)
...
replComponent
  ...
  distPref = do
    ...
    let verbHandles = verbosityHandles verbosity
        ...
        lib' = lib{libBuildInfo = libbi{cSources = cSources libbi ++ map (`ExtraSourcePkg` []) extras}}
  ...
  _ =
    do
      ...
      case comp of
        ...
        CExe exe -> do
          let ebi = buildInfo exe
              exe' = exe{buildInfo = ebi{cSources = cSources ebi ++ map (`ExtraSourcePkg` []) extras}}
